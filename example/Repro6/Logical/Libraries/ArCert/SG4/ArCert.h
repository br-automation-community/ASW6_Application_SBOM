...
typedef enum ArCertSecureKeySizeEnum
{	arCERT_SECURE_KEY_SIZE_2048_BITS = 1,
	arCERT_SECURE_KEY_SIZE_4096_BITS = 2
} ArCertSecureKeySizeEnum;
...
typedef struct ArCertCsrDetailsType
{	plcstring Name[256];
	plcstring Subject[256];
	signed long Version;
} ArCertCsrDetailsType;
...
typedef struct ArCertCsrCreate
{
	/* VAR_INPUT (analog) */
	plcstring Name[256];
	plcstring PrivateKeyName[256];
	plcstring PrivateKeyPassword[256];
	enum ArCertSecureKeySizeEnum KeySize;
	plcstring CountryCode[10];
	plcstring StateProvince[514];
	plcstring Locality[514];
	plcstring Organization[256];
...
typedef struct ArCertCsrExport
{
	/* VAR_INPUT (analog) */
	plcstring Name[256];
	unsigned long Data;
...
typedef struct ArCertCsrGetDetails
{
	/* VAR_INPUT (analog) */
	plcstring Name[256];
	unsigned long Index;
	/* VAR_OUTPUT (analog) */
	signed long StatusID;
	struct ArCertCsrDetailsType Details;
	/* VAR (analog) */
	struct ArCertAsyncInternalType Internal;
	/* VAR_INPUT (digital) */
	plcbit Execute;
	...
	plcbit Busy;
	plcbit Error;
} ArCertCsrGetDetails_typ;

typedef struct ArCertCsrDelete
{
	/* VAR_INPUT (analog) */
	plcstring Name[256];
	/* VAR_OUTPUT (analog) */
...
_BUR_PUBLIC void ArCertCsrCreate(struct ArCertCsrCreate* inst);
_BUR_PUBLIC void ArCertCsrExport(struct ArCertCsrExport* inst);
_BUR_PUBLIC void ArCertCsrGetDetails(struct ArCertCsrGetDetails* inst);
_BUR_PUBLIC void ArCertCsrDelete(struct ArCertCsrDelete* inst);
