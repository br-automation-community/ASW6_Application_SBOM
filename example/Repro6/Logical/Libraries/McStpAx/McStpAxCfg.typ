TYPE
	...
	McSAHStpAxParCkEnum :
		...
		mcAHMRPSTAPC_NOT_USE := 0, (*Not used - Axis parameterization check is not performed*)
		mcAHMRPSTAPC_USE := 1 (*Used - Axis parameterization check is performed*)
		);
	McSAHStpAxParCkType : STRUCT (*Activate check if axis parameterization has change for restore position*)
		Type : McSAHStpAxParCkEnum; (*Restore position axis scaling check selector setting*)
	END_STRUCT;
	...
	McSAHType : STRUCT (*Homing mode and parameters which can be used within the application program as preconfigured setting*)
		...
		StpAxParCk : McSAHStpAxParCkType; (*Activate check if axis parameterization has change for restore position*)
	...
	END_STRUCT;
	McSAMELOvercurrentErrReacEnum :
		( (*Reaction if the bit OvercurrentError0x is set*)
		mcSAMELOCER_ERRORSTOP := 0, (*Errorstop - Axis will perform an error stop based on the configured behavior at Stop reaction - Drive error*)
		mcSAMELOCER_WAR := 1, (*Warning - A warning will be entered into the logger*)
		mcSAMELOCER_NONE := 2 (*None - No reaction is triggered*)
		);
	McSAMELOvrTmpErrReacEnum :
		( (*Reaction if the bit OvertemperatureError0x is set*)
		mcSAMELOTER_ERRORSTOP := 0, (*Errorstop - Axis will perform an error stop based on the configured behavior at Stop reaction - Drive error*)
		mcSAMELOTER_WAR := 1, (*Warning - A warning will be entered into the logger*)
		mcSAMELOTER_NONE := 2 (*None - No reaction is triggered*)
		);
	McSAMELType : STRUCT (*Limit values that result in a stop reaction when exceeded*)
		...
		OvercurrentErrorReaction : McSAMELOvercurrentErrReacEnum; (*Reaction if the bit OvercurrentError0x is set*)
		OvertemperatureErrorReaction : McSAMELOvrTmpErrReacEnum; (*Reaction if the bit OvertemperatureError0x is set*)
	END_STRUCT;
	McSAJFEnum :
